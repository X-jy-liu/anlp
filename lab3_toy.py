...
def encode_token_freq(data, vocab):
    ...
    for exp in data:
        ...
        exp_vector = [len(re.findall(rf'\b{re.escape(word)}\b', exp, flags=re.IGNORECASE))
                  for word in vocab]
        exp_vector = np.array(exp_vector, dtype=float).reshape(len(vocab))
        # exp_vector = np.array([exp.count(word_type) for word_type in vocab]).reshape(len(vocab)) # students replace this
...
def feature_probabilities(training_x, training_y, alpha):
    ...
    for c in set(training_y):       #for each category
        ...
        c_data = np.zeros(num_feat)
        for i in c_exps:
            c_data = c_data + training_x[i]     # students replace this
        c_data += alpha
...
def nb_posterior_log_probability(exp, c, c_probs, f_probs):
    ...
    # print(c_probs)
    # print(exp)
    # posterior = np.log(c_probs[c]*f_probs[c] / (exp/np.sum(exp)))
    print(f'The feature prob is {f_probs}')
    print(f'The frequency of words is {exp}')
    posterior = c_probs[c] + np.multiply(f_probs[c], exp).sum()
    # print(c_probs[c])
...
print(nb_classify(test_word_features, nb_model))
...
########## LOGISTIC REGRESSION ##########
print('='*30 + 'Look at the Logistic Regression' + '='*30)
